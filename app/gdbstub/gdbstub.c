...
static void ATTR_GDBFN gdb_exception_handler(struct exception_frame *frame, uint32_t cause) {
...
static void ATTR_GDBFN uart_hdlr(void *frame) {
...
ets_printf("gdbstub_init1\n");
#if GDBSTUB_REDIRECT_CONSOLE_OUTPUT
  os_install_putc1(gdb_semihost_putchar1);
#endif
ets_printf("gdbstub_init2\n");
#if GDBSTUB_CTRLC_BREAK
  install_uart_hdlr();
#endif
ets_printf("gdbstub_init3\n");
  install_exceptions();
ets_printf("gdbstub_init4\n");
  gdbstub_init_debug_entry();
ets_printf("gdbstub_init5\n");
#if GDBSTUB_BREAK_ON_INIT
  gdbstub_do_break();
#endif
ets_printf("gdbstub_init6\n");
}
